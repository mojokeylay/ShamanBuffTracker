...
resetBtn:SetScript("OnClick", function() StaticPopup_Show("SHAMAN_TRACKER_CONFIRM_RESET") end)
...
local function GetIconString(id)
    local texture = C_Spell.GetSpellTexture(id) or id
...
function frame:UpdateStatus()
    ...
    if ShamanBuffTrackerDB.trackMainHand and not hasMH then 
        table.insert(missing, "|T136014:16:16:0:0|t Main-hand imbue") 
    end
    if ShamanBuffTrackerDB.isEnhancement and not hasOH then 
        table.insert(missing, "|T135814:16:16:0:0|t Off-hand imbue") 
    end

    if ShamanBuffTrackerDB.trackLightningShield then
        if not PlayerHasBuff("Lightning Shield") then
            table.insert(missing, GetIconString(192106) .. "Lightning Shield")
        end
    end

    if ShamanBuffTrackerDB.trackEarthShield then
        if not PlayerHasBuff("Earth Shield") then
            table.insert(missing, GetIconString(974) .. "Earth Shield")
        end
    end

    if ShamanBuffTrackerDB.trackSkyfury then
        if not PlayerHasBuff("Skyfury") then
